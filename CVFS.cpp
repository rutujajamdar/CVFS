...
#include<stdlib.h>
#include<string.h>
#include<unistd.h>  // macro of file handling
#include<iostream>
//#include<io.h>

#define MAXINODE 50  // number of files we can create is 50
...
#define REGULAR 1  // project support
...
typedef struct filetable
{
    int readoffset;  
    int writeoffset;  
    int count;  // always 1 ---> increase when any of process generate chile process
    int mode; // 1 2 3
    PINODE ptrinode;
...
int CreateFile(char *name ,int permission)
{
    ...
    UFDTArr[i].ptrfiletable->ptrinode->RefeerenceCount=1;
...
int WriteFile(int fd,char *arr,int isize)
{
    if(((UFDTArr[fd].ptrfiletable->mode)!=WRITE) && ((UFDTArr[fd].ptrfiletable->mode)!=READ+WRITE))   
     ...
     // 375  ----->1 parameter append data at end at file
...
int OpenFile(char *name,int mode)
{
    ...
    UFDTArr[i].ptrfiletable->ptrinode=temp;
    (UFDTArr[i].ptrfiletable->ptrinode->RefeerenceCount)++;
...
void CloseFileByName(int fd)
{
    ...
    (UFDTArr[fd].ptrfiletable->ptrinode->RefeerenceCount)--;
}
...
int fstat_file(int fd)
{
    ...
    printf("File Size : %d\n",temp->FileSize);
    printf("Actaul File Size : %d\n",temp->FileActualSize);
    printf("Link Count : %d\n",temp->LinkCount);
    printf("Reference Count : %d\n",temp->RefeerenceCount);
...
int main()
{
    ...
    InitializeSuperBlock();  // auxilary data 
    CreateDILB();
    while (1)
    {
        fflush(stdin);
        ...
        // strtok
            if(count==1)
            {
                ...
                {
                    CloseAllFile();
                    printf("All Files cloesd successfully \n");
                    continue;
                }
            ...
            else if(count==2)
            {
                ...
                else if(strcmp(command[0],"write")==0)
                {
                    ...
                    if(ret==0)
                    {
                        printf("ERROR : Imcorrect Parameter\n");
                        continue;
                    }
